...
#define FILL_EVEN_INDICES_OF_POSITION_ENCODING(pe, s) {\
    for (cc_tokenizer::string_character_traits<char>::size_type i = 0; i < s.getShape().getN(); i++)\
    {\
...
#define FILL_ODD_INDICES_OF_POSITION_ENCODING(pe, c) {\
    for (cc_tokenizer::string_character_traits<char>::size_type i = 0; i < c.getShape().getN(); i++)\
    {\
...
{\
    ...
    /*ptr[i] = ret->index;*/\
    ptr[i] = v(p.get_token_by_number(i + 1));\
}\
...
#define BUILD_POSITION_ENCODING_FOR_LINE_BATCH_SIZE(p, is, dt, dm, pe, t) {\
    try\
    {\
        p = Numcy::arange<t, t>((t)0.0, (t)is.getShape().getNumberOfColumns()/*[NUMCY_DIMENSIONS_SHAPE_COLUMNS]*/, (t)1.0, DIMENSIONS{1, is.getShape().getNumberOfColumns()/*[NUMCY_DIMENSIONS_SHAPE_COLUMNS]*/, NULL, NULL});\
